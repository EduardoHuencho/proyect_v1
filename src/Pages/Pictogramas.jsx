...
function Pictogramas() {
    ...
    useEffect(() => {
        const configurarVoz = () => {
            ...
            const mejorVoz = vocesEspanol.find(voz => 
    ...
    const pictogramasFiltrados = categoriaSeleccionada === 'Todos'
        ? PictogramasList
        : PictogramasList.filter(item => item.category === categoriaSeleccionada);
        ...
        <div className="border border-gray-200 bg-white rounded-2xl h-[125px] mb-4 flex items-center justify-between p-3 gap-3 shadow-sm overflow-hidden shrink-0">
          ...
          >
            {frase.length === 0 ? (
              ...
              frase.map((pic, index) => (
                <div key={index} className="p-1.5 border border-gray-200 rounded-xl bg-gray-50 text-center w-16 sm:w-20 shrink-0 animate-fade-in">
                  <span className="text-xl sm:text-2xl">{pic.icon}</span>
                  <p className="text-[10px] sm:text-xs font-semibold truncate text-gray-700">{pic.label}</p>
                </div>
              ))
            )}
          </div>
